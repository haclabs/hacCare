...
const EnhancedDebriefModal: React.FC<EnhancedDebriefModalProps> = ({ historyRecord, onClose }) => {
  ...
  const loadStudentActivities = async () => {
    try {
      ...
      // Querying live tables returns wrong (or empty) data once the simulation
      // tenant has been reset — the reset deletes all clinical entries so the
      // live tables are empty for the session that just ran.
      ...
      // Fall back to a live query for older records that have no snapshot yet,
      // or for simulations that completed but haven't been reset.
      // Pass started_at as the time anchor so a later reset (which NULLs
      // simulation_active.starts_at) doesn't corrupt the results.
