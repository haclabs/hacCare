...
export const PatientDetail: React.FC<PatientDetailProps> = ({ patient, onBack }) => {
  ...
  const handleSaveAssessment = async (assessmentData: any) => {
    try {
      // For now, we'll add assessments as notes
      // In a real app, you'd have a separate assessments table
      const assessmentNote = {
        ...
        content: `${assessmentData.assessment_type.charAt(0).toUpperCase() + assessmentData.assessment_type.slice(1)} Assessment: ${assessmentData.assessment_notes}`,
        priority: assessmentData.priority_level === 'critical' ? 'Critical' : 
                 assessmentData.priority_level === 'urgent' ? 'High' : 'Medium'
      };
