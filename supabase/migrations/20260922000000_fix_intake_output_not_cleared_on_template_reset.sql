-- ============================================================================
-- FIX: reset_simulation_with_template_updates never cleared intake/output
-- ============================================================================
-- CLAUDE.md states that a new patient_* clinical table needs a
-- `DELETE FROM <table> WHERE tenant_id = v_tenant_id` in BOTH
-- reset_simulation_for_next_session AND reset_simulation_with_template_updates,
-- or entries accumulate across resets. patient_intake_output_events had it in
-- the first and not the second -- the only table of 30 that diverged:
--
--   reset_simulation_for_next_session        30 DELETE targets
--   reset_simulation_with_template_updates   29 DELETE targets
--   difference                               patient_intake_output_events
--
-- SYMPTOM: resetting a simulation via "reset with template updates" (the path
-- an instructor takes after editing a template mid-semester) left the previous
-- cohort's intake and output entries on the chart. The plain reset path was
-- unaffected, which is why this survived -- the two paths look identical to an
-- instructor and only one leaked.
--
-- The fix mirrors the working function exactly, including its second delete by
-- patient_id: rows created before tenant_id was backfilled carry only
-- patient_id, so the tenant_id predicate alone misses them.
--
-- Guarded against recurrence by src/services/simulation/simulationWiring.test.ts,
-- which asserts the two functions delete from the same set of tables.
-- ============================================================================
...
BEGIN
  ...
  -- Intake/output: deleted both ways, mirroring reset_simulation_for_next_session.
  -- Rows created before the tenant_id backfill carry only patient_id.
