-- SECURE FIX: Handle dependencies properly to eliminate infinite recursion
-- Copy and paste this into Supabase SQL Editor

-- Step 1: Drop dependent policies first, then functions
DROP POLICY IF EXISTS "Users can view their tenant" ON tenants;
DROP POLICY IF EXISTS "Users can view their own tenant" ON tenants;
DROP POLICY IF EXISTS "tenant_isolation_policy" ON tenants;

-- Drop any other policies that might depend on the functions
...
-- Step 2: Now safely drop the problematic functions
...
-- Step 3: Drop all existing problematic policies on tenant_users
...
-- Step 4: Create SIMPLE tenant_users policies that don't reference tenant_users
...
-- Step 5: Create a materialized view to cache tenant relationships (no RLS recursion)
DROP MATERIALIZED VIEW IF EXISTS user_tenant_cache CASCADE;
CREATE MATERIALIZED VIEW user_tenant_cache AS
SELECT 
  user_id,
  tenant_id,
  role,
...
-- Step 6: Create function to refresh the cache (call this when tenant_users changes)
...
-- Step 7: Create secure helper functions using the cache (no recursion)
...
-- Step 8: Create secure policies for other tables using the cache
...
-- Step 9: Secure tenant policies using the cache
...
-- Step 10: Apply same pattern to patients table (secure tenant isolation)
DO $$
BEGIN
...
-- Step 11: Apply to other tables that might have tenant_id
DO $$
BEGIN
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'patient_medications') THEN
    ...
    EXECUTE 'CREATE POLICY "patient_medications_tenant_isolation" ON patient_medications 
             FOR ALL USING (
               is_super_admin_secure() OR 
               EXISTS (
                 SELECT 1 FROM patients p 
                 WHERE p.id = patient_medications.patient_id 
                 AND p.tenant_id = get_user_tenant_secure()
               )
             )';
  END IF;
END $$;

-- Step 12: Set up automatic cache refresh trigger
...
-- Step 13: Initial cache population
SELECT refresh_user_tenant_cache();

-- Step 14: Grant minimal necessary permissions
GRANT SELECT ON user_tenant_cache TO authenticated;
