-- ===========================================================================
-- COMPLETE FIX: Snapshot Functions with Correct Table Names
-- ===========================================================================
-- Purpose: Run this file in Supabase SQL Editor to create/update snapshot functions
-- Fixes applied:
--   1. Uses patient_medications (not medications)
--   2. Only queries tables that exist in your database
--   3. Uses COALESCE to handle empty results gracefully
-- ===========================================================================

-- Tables included in snapshot (all patient-related data):
--   ✅ patients
--   ✅ patient_medications
--   ✅ patient_vitals
--   ✅ patient_notes
...
--   1. fix_tenant_type_constraint.sql
--   2. fix_subdomain_constraint.sql
-- ===========================================================================

-- This file contains the two critical snapshot functions:
-- - save_template_snapshot
-- - restore_snapshot_to_tenant

-- ---------------------------------------------------------------------------
-- Save snapshot of template tenant data (FIXED: uses patient_medications)
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  -- Includes all patient-related tables that exist in your database
...
-- ---------------------------------------------------------------------------
-- Restore snapshot data to a tenant (FIXED: uses patient_medications)
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  -- Restore patient wounds (need to track wound ID mapping)
  ...
  -- Restore wound assessments (using wound mapping)
...
DO $$ 
BEGIN
  RAISE NOTICE '✅ Snapshot functions created/updated successfully';
  RAISE NOTICE 'Functions ready: save_template_snapshot, restore_snapshot_to_tenant';
  RAISE NOTICE 'Now you can save snapshots and launch simulations!';
END $$;
