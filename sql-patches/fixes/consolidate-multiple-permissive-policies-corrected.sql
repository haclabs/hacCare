-- =============================================================================
-- CONSOLIDATE MULTIPLE PERMISSIVE POLICIES - CORRECTED VERSION
-- =============================================================================
...
CREATE POLICY "tenants_consolidated_update" ON public.tenants
    ...
    USING (
        ...
        OR
        -- Tenant owners can update their tenant (admin role within tenant)
        id IN (
            SELECT tenant_id FROM public.tenant_users 
...
-- Drop the duplicate constraint on tenants.subdomain (keep unique_subdomain)
ALTER TABLE public.tenants DROP CONSTRAINT IF EXISTS tenants_subdomain_key;
-- Keep unique_subdomain as it's the primary constraint we want
