-- =====================================================
-- FIX: RESET SIMULATION UUID CASTING ERROR
-- =====================================================
-- ISSUE: 'operator does not exist: text = uuid' error
-- CAUSE: Comparing text patient_id from JSONB with UUID column
-- FIX: Cast patient_id from JSONB to UUID in comparison
-- =====================================================
...
BEGIN
  ...
  IF v_snapshot ? 'patients' THEN
    ...
    WITH template_patients AS (
      SELECT 
        (p->>'patient_id')::uuid as patient_id,  -- FIXED: Cast to UUID instead of text
    ...
    updated AS (
      ...
      AND p.patient_id = tp.patient_id  -- NOW: UUID = UUID comparison (FIXED!)
      RETURNING p.id
    )
...
FIXED: UUID casting issue that caused "operator does not exist: text = uuid" error
...
DO $$
BEGIN
  RAISE NOTICE '🔧 MIGRATION 017: Fixed UUID casting in reset_simulation_for_next_session function';
  RAISE NOTICE '   - Changed (p->>"patient_id")::text to (p->>"patient_id")::uuid';
  RAISE NOTICE '   - This fixes the "operator does not exist: text = uuid" error';
  RAISE NOTICE '   - Patient ID comparisons now work correctly';
END $$;