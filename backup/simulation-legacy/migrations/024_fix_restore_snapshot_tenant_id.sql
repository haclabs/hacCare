-- =====================================================
-- FIX: Add tenant_id to diabetic_records INSERT in restore_snapshot_to_tenant
-- =====================================================
-- ERROR: column "tenant_id" does not exist (error code 42703)
-- When calling launch_simulation -> restore_snapshot_to_tenant
-- 
-- ISSUE: diabetic_records table HAS tenant_id column (NOT NULL)
-- But restore function's INSERT doesn't include it
--
-- SOLUTION: Add tenant_id to INSERT statement
-- Use p_tenant_id (the new simulation tenant being restored to)
-- =====================================================
  ...
  IF p_snapshot ? 'diabetic_records' AND jsonb_array_length(p_snapshot->'diabetic_records') > 0 THEN
    ...
    LOOP
      INSERT INTO diabetic_records (
        patient_id,
        tenant_id,  -- ✅ ADDED: Required NOT NULL column
        blood_glucose,
        insulin_dose,
        carbs_consumed,
        recorded_at,
      ...
      VALUES (
        (v_patient_mapping->(v_record->>'patient_id'))::uuid,
        p_tenant_id,  -- ✅ ADDED: Use new simulation tenant ID
...
Fixed to include tenant_id for diabetic_records.
Only restores tables that actually exist (11 tables).';
