-- ============================================================================
-- FIX PATIENT MAPPING IN RESTORE_SNAPSHOT_TO_TENANT
-- ============================================================================
-- Migration: Fix non-deterministic patient mapping when preserve_barcodes=true
-- Author: GitHub Copilot
-- Date: 2026-04-07
-- ============================================================================
-- Root cause:
--   During simulation LAUNCH, all patients are created inside one transaction.
--   PostgreSQL's NOW() returns the transaction start time, so every patient in
--   the simulation gets the EXACT SAME created_at timestamp.
--
--   On RESET, restore_snapshot_to_tenant (preserve_barcodes=true) mapped
--   snapshot patients to simulation patients using:
--
--       SELECT id FROM patients WHERE tenant_id = p_tenant_id
--       ORDER BY created_at
--       OFFSET v_count LIMIT 1;
--
--   When two or more patients share the same created_at, ORDER BY created_at
--   is non-deterministic. OFFSET 0 and OFFSET 1 may return any patient.
--   Result: snapshot patient A gets mapped to simulation patient B (and
--   vice-versa), so doctors_orders / vitals / etc. are either cross-mapped
--   or skipped entirely – explaining "only the last patient's doctors orders
--   appear after reset."
--
-- Fix:
--   Replace positional OFFSET mapping with demographics matching:
--     first_name + last_name + date_of_birth
--   This is the canonical approach documented in copilot-instructions.md
...
COMMENT ON FUNCTION public.restore_snapshot_to_tenant IS 
  'Restores snapshot data to a tenant. FIXED: Patient mapping uses demographics (first/last/dob) instead of positional ORDER BY created_at OFFSET — the old approach was non-deterministic because simulation patients share the same created_at from being inserted in one transaction.';

SELECT '✅ Migration Complete' as status, 
       'Fixed patient mapping: demographics-based instead of positional OFFSET' as description;
