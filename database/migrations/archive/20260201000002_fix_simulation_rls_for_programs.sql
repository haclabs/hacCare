...
-- ============================================================================
-- 1. DROP OLD AND NEW POLICIES (IDEMPOTENT)
-- ============================================================================
...
-- ============================================================================
-- 2. CREATE NEW PROGRAM-AWARE SELECT POLICIES
-- ============================================================================

-- SIMULATION_TEMPLATES: Instructors see only templates for their programs
...
-- Add UPDATE policy for templates
...
-- Add DELETE policy for templates
...
-- SIMULATION_ACTIVE: Instructors see only active simulations for their programs
...
-- (Already dropped above - no need to drop again)
...
-- ============================================================================
-- 3. UPDATE INSERT POLICY TO REQUIRE VALID PROGRAM CATEGORIES
-- ============================================================================
...
CREATE POLICY active_insert_policy
  ...
  WITH CHECK (
    EXISTS (
      CREATE UPDATE POLICY FOR MANAGING SIMULATIONS
-- ============================================================================
...
-- ============================================================================
-- 4. ADD UPDATE POLICY FOR MANAGING SIMULATIONS
-- ============================================================================

-- Drop old update policy if exists
DROP POLICY IF EXISTS active_update_policy ON simulation_active;

-- Create program-aware update policy
    ...
    (
      ...
      AND (
        ...
        EXISTS (
          ...
          WHERE up_prog.user_id = auth.uid()
            AND prog.code = ANY(simulation_active.primary_categories)
      CREATE DELETE POLICY FOR REMOVING SIMULATIONS
-- ============================================================================
...
-- ============================================================================
-- 5. ADD DELETE POLICY FOR REMOVING SIMULATIONS
-- ============================================================================

-- Drop old delete policy if exists
DROP POLICY IF EXISTS active_delete_policy ON simulation_active;

-- Create program-aware delete policy
...
-- ============================================================================
-- 6. ENSURE STUDENT POLICIES REMAIN INTACT
-- ============================================================================

-- Drop existing student policy if it exists
DROP POLICY IF EXISTS templates_select_student ON simulation_templates;

-- Re-create student SELECT policy for templates (unchanged)
