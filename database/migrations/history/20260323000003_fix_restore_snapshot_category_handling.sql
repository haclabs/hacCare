-- ============================================================================
-- FIX SNAPSHOT RESTORATION FOR CATEGORY FIELD
-- ============================================================================
-- Migration: Update restore_snapshot_to_tenant to handle old medication format
-- Author: GitHub Copilot
-- Date: 2026-03-23
-- ============================================================================
-- Purpose: Ensure medications from old snapshots (with is_prn) get proper category
--          Old snapshots: {is_prn: true} → New schema: {category: 'prn'}
-- ============================================================================
...
DECLARE
  ...
  v_column_type text;
  v_udt_name text;
  v_computed_category text;  -- ⭐ NEW: For computing category from is_prn
BEGIN
  ...
  -- [STEP 1: Patient mapping code unchanged - keeping original logic]
  ...
  LOOP
    ...
    IF jsonb_array_length(v_table_data) > 0 THEN
      ...
      LOOP
        v_columns := ARRAY[]::text[];
        v_values := ARRAY[]::text[];
        v_computed_category := NULL;  -- ⭐ Reset for each record
        
        -- ⭐ NEW: For patient_medications, compute category from is_prn if category missing
        IF v_actual_table_name = 'patient_medications' THEN
          IF NOT (v_record ? 'category') AND (v_record ? 'is_prn') THEN
            -- Old snapshot format: compute category from is_prn
            v_computed_category := CASE 
              WHEN (v_record->>'is_prn')::boolean = true THEN 'prn'
              ELSE 'scheduled'
            END;
            RAISE NOTICE '🔄 Computing category from is_prn: % → %', 
              v_record->>'is_prn', v_computed_category;
          END IF;
        END IF;
        ...
        -- [Patient mapping logic unchanged]
        ...
        -- ⭐ NEW: Add computed category BEFORE processing other columns
        IF v_computed_category IS NOT NULL THEN
          v_columns := array_append(v_columns, 'category');
          v_values := array_append(v_values, quote_literal(v_computed_category));
        END IF;
        ...
        FOR v_col IN 
          SELECT key, value 
          FROM jsonb_each(v_record)
          WHERE key NOT IN ('id', 'tenant_id', 'patient_id', 'created_at', 'updated_at', 'is_prn', 'category')  -- ⭐ Skip both is_prn and category
        LOOP
...
COMMENT ON FUNCTION public.restore_snapshot_to_tenant IS 
  'Restores snapshot data to a tenant. Handles backward compatibility for medication category field by computing from is_prn boolean in old snapshots.';

-- Verification
SELECT '✅ Migration Complete' as status, 
       'restore_snapshot_to_tenant now handles category field backward compatibility' as description;
