-- Migration: Track which named template state an active simulation was last reset to
-- Date: 2026-09-12
-- Companion to 20260912000000/20260912000001 (simulation_template_states +
-- p_state_id reset params). Lets the UI show a "State: Week 2" badge on the
-- active simulation card instead of the instructor having to remember.

ALTER TABLE public.simulation_active
  ADD COLUMN current_state_id uuid REFERENCES public.simulation_template_states(id) ON DELETE SET NULL;
...
-- Re-deploy both reset functions to record current_state_id on the simulation
-- row (same signature, no overload/ambiguity concerns this time).
