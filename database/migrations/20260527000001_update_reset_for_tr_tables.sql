-- Migration: Redeploy both reset functions to handle TR module tables
-- Date: 2026-05-27
--
-- Changes:
--   reset_simulation_for_next_session:
--     • Adds DELETE for 6 new TR tables (is_baseline = false only)
--     • Strips TR tables from snapshot before restore_snapshot_to_tenant
--       (prevents duplication of baseline rows already preserved in-place)
--
--   reset_simulation_with_template_updates:
--     • Adds DELETE for patient_system_assessments (was ON HOLD — now active)
--     • Adds DELETE for 6 new TR tables (is_baseline = false only)
--     • Strips PSA + TR tables from snapshot before restore_snapshot_to_tenant
--
-- IMPORTANT: Editing database/functions/*.sql locally does NOT update the live DB.
-- This migration creates the authoritative deployed version.
...
BEGIN
  ...
  -- =====================================================
  -- STEP 1: SAVE PATIENT & MEDICATION BARCODE IDs
  -- =====================================================
  ...
  WHERE tenant_id = v_tenant_id AND is_baseline = false;
  GET DIAGNOSTICS v_count = ROW_COUNT;
  ...
  WHERE patient_id::uuid IN (SELECT id FROM patients WHERE tenant_id = v_tenant_id);
  GET DIAGNOSTICS v_count = ROW_COUNT;
  ...
  EXCEPTION WHEN undefined_column THEN
    DELETE FROM patient_intake_output_events
    WHERE patient_id IN (SELECT id FROM patients WHERE tenant_id = v_tenant_id);
    GET DIAGNOSTICS v_count = ROW_COUNT;
  ...
  -- Strip tables where baseline rows are preserved in-place (avoids duplication)
  ...
  RAISE NOTICE '📋 Stripped PSA + TR tables from snapshot (baseline rows preserved in-place)';

  RAISE NOTICE '👥 Keeping patients in snapshot for ID mapping';
...
COMMENT ON FUNCTION reset_simulation_for_next_session IS
  'Reset simulation for next session — preserves patient/medication barcodes, sets status to pending (manual start required). Deletes student TR module entries; preserves instructor baseline TR data.';
...
BEGIN
  ...
  -- STEP 0: Validate patient list hasn't changed
  ...
  IF v_template_meds IS NOT NULL THEN
    ...
    FOR i IN 0..jsonb_array_length(v_template_meds) - 1 LOOP
      ...
      SELECT pat.value->>'first_name', pat.value->>'last_name', pat.value->>'date_of_birth'
      ...
      IF NOT v_med_exists THEN
        BEGIN
          INSERT INTO patient_medications (
            ...
            CASE WHEN v_template_med->>'end_date' IS NOT NULL THEN (v_template_med->>'end_date')::date ELSE NULL END,
            CASE WHEN v_template_med->>'next_due' IS NOT NULL THEN (v_template_med->>'next_due')::timestamptz ELSE NULL END,
  ...
  -- Strip tables whose baseline rows are preserved in-place
  v_snapshot := v_snapshot - 'patient_medications';
  v_snapshot := v_snapshot - 'patient_system_assessments';
  v_snapshot := v_snapshot - 'tr_screening_entries';
  v_snapshot := v_snapshot - 'tr_active_living_profiles';
...
COMMENT ON FUNCTION reset_simulation_with_template_updates IS
  'Smart template sync — property-based medication matching. Now also deletes student PSA + TR module entries while preserving instructor baseline rows.';
